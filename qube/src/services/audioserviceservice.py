...
class audioserviceService:
    ...
    def find_by_id(self, entity_id):
        ...
        data = audioservice.query.get(entity_id)
        if data is None:
            raise audioserviceServiceError(
                'audioservice {} not found'.format(entity_id),
    ...
    def get_all(self):
        list = []
        data = audioservice.query.filter(
            audioservice.tenantId == self.auth_context.tenant_id)
    ...
    def update(self, model, entity_id):
        ...
        record = audioservice.query.get(entity_id)  # audioservice is a mongo class
        if record is None:
            raise audioserviceServiceError(
                'audioservice {} not found'.format(entity_id),
                ErrorCodes.NOT_FOUND)
